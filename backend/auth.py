...
SECRET_KEY = "super_secret_rbac_dashboard_key"
ALGORITHM = "HS256"
...
async def get_current_user(token: str = Depends(oauth2_scheme), session: Session = Depends(db.get_db)):
    ...
    user = session.query(models.User).filter(models.User.username == token_data.username).first()
    if user is None:
        raise credentials_exception
    return user
...
def check_manager(user: models.User = Depends(get_current_user)):
